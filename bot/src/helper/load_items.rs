...
use crate::database::item::{ActiveModel as ItemActiveModel, Entity as Item};
...
pub async fn load_items_from_json(db: &DatabaseConnection) -> Result<()> {
    ...
    // Read the JSON file
    let json_content = read_file_as_string("./json/items/items.json")
        .context("Failed to read items JSON file")?;

    // Parse the JSON
    let items_json: ItemsJson = serde_json::from_str(&json_content)
    ...
    for item_json in items_json.items {
        ...
        let item = ItemActiveModel {
            ...
            max_number: Set(item_json.max_number),
