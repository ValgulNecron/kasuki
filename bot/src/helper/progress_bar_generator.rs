...
pub fn generate_all_progress_bar_images() -> Result<()> {
    ...
    for percent in (0..=100).step_by(10) {
        match generate_progress_bar_image(percent, None) {
            Ok(_) => debug!("Generated progress bar image for {}% with default color", percent),
            Err(e) => {
                error!("Failed to generate progress bar image for {}%: {}", percent, e);
                return Err(e).context(format!("Failed to generate progress bar image for {}%", percent));
            }
        }
    }
