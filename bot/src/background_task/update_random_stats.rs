...
pub async fn update_random_stats_launcher(
	...
	debug!("Random stats update interval configured for {} seconds", task_intervals.random_stats_update);
	...
	loop {
		...
		info!("Starting random stats update cycle #{} at {}", update_count, current_time);
		...
		match update_random_stats(anilist_cache.clone()).await {
			Ok(stats) => {
				info!("Random stats update cycle #{} completed successfully", update_count);
				debug!("Updated random stats: anime_last_page={}, manga_last_page={}", 
					stats.anime_last_page, stats.manga_last_page);

				// Reset failure counter on success
				if consecutive_failures > 0 {
					info!("Random stats update recovered after {} consecutive failures", consecutive_failures);
					consecutive_failures = 0;
				}
			},
			Err(err) => {
				consecutive_failures += 1;
				error!("Random stats update cycle #{} failed: {:#}", update_count, err);
				warn!("This is consecutive failure #{} for random stats updates", consecutive_failures);
				...
				if consecutive_failures > MAX_CONSECUTIVE_FAILURES {
					let delay = base_retry_delay.mul_f32(1.5_f32.powi(consecutive_failures as i32 - 3));
					warn!("Multiple consecutive random stats update failures detected. Implementing backoff strategy.");
		...
		debug!("Next random stats update scheduled in {} seconds", task_intervals.random_stats_update);
...
pub async fn update_random_stats(
	...
	debug!("Attempting to load random stats from file: {}", RANDOM_STATS_PATH);

	// Try to load random stats from a JSON file.
	let mut random_stats: RandomStat = match std::fs::read_to_string(RANDOM_STATS_PATH) {
		Ok(stats) => {
			trace!("Successfully read random stats file, content length: {} bytes", stats.len());
			debug!("Parsing random stats JSON content");

			match serde_json::from_str::<RandomStat>(&stats) {
				Ok(parsed_stats) => {
					debug!("Successfully parsed random stats: anime_last_page={}, manga_last_page={}", 
						parsed_stats.anime_last_page, parsed_stats.manga_last_page);
					parsed_stats
				},
				Err(e) => {
					warn!("Failed to parse random stats JSON from {}: {}", RANDOM_STATS_PATH, e);
					debug!("First 100 characters of file content: {}", stats.chars().take(100).collect::<String>());
					return Err(anyhow::anyhow!(e)
						.context(format!("Failed to parse random stats JSON from {}", RANDOM_STATS_PATH)));
				}
			}
		},
		Err(err) => {
			...
			if err.kind() == std::io::ErrorKind::NotFound {
				info!("Random stats file not found ({}), using default values", RANDOM_STATS_PATH);
			} else {
				warn!("Could not read random stats file ({}), using default values: {}", RANDOM_STATS_PATH, err);
				debug!("Error kind: {:?}", err.kind());
			}

			let default_stats = RandomStat::default();
			debug!("Using default values: anime_last_page={}, manga_last_page={}", 
	...
	trace!("Current stats before update: anime_last_page={}, manga_last_page={}", 
	...
	debug!("update_random completed in {} ms", elapsed.num_milliseconds());
	debug!("Updated stats: anime_last_page={}, manga_last_page={}", 
		random_stats.anime_last_page, random_stats.manga_last_page);
	...
	match std::fs::write(RANDOM_STATS_PATH, &random_stats_json) {
		Ok(_) => debug!("Successfully wrote random stats to file: {}", RANDOM_STATS_PATH),
		Err(e) => {
			warn!("Failed to write random stats to file {}: {}", RANDOM_STATS_PATH, e);
			debug!("Error kind: {:?}", e.kind());
			return Err(anyhow::anyhow!(e)
				.context(format!("Failed to write random stats to file {}", RANDOM_STATS_PATH)));
		}
	}

	// Log successful update
	info!("Successfully updated random stats: anime_last_page={}, manga_last_page={}", 
		random_stats.anime_last_page, random_stats.manga_last_page);
...
async fn update_random(
	...
	debug!("Initial stats: anime_last_page={}, manga_last_page={}", 
		random_stats.anime_last_page, random_stats.manga_last_page);
	...
	info!("Starting anime statistics update from page {}", random_stats.anime_last_page);
	...
	while has_more_pages && anime_failures < MAX_FAILURES {
		trace!("Calling update_page for anime (page {})", random_stats.anime_last_page);
		...
		trace!("update_page for anime completed in {} ms", page_elapsed.num_milliseconds());

		if has_more_pages {
			pages_updated += 1;
			debug!("Successfully updated anime page {}, moving to next page", random_stats.anime_last_page - 1);

			if anime_failures > 0 {
				debug!("Reset anime failure counter from {} to 0 after successful update", anime_failures);
			...
			anime_failures += 1;
			warn!("Anime update attempt failed for page {}, failure {}/{}", 
			...
			if anime_failures < MAX_FAILURES {
				...
				warn!("Reached maximum anime update failures ({}), moving to manga updates", MAX_FAILURES);
			}
		}
	...
	if anime_failures >= MAX_FAILURES {
		error!("Exceeded maximum failures ({}) for anime statistics update", MAX_FAILURES);
		warn!("Some anime statistics may not be up to date");
	}

	let anime_elapsed = chrono::Utc::now() - anime_start_time;
	info!("Completed anime statistics update, processed {} pages in {} seconds", 
		pages_updated, anime_elapsed.num_seconds());
	debug!("Average time per anime page: {} ms", 
		if pages_updated > 0 { anime_elapsed.num_milliseconds() / pages_updated as i64 } else { 0 });

	// Update manga statistics
	info!("Starting manga statistics update from page {}", random_stats.manga_last_page);
	...
	while has_more_pages && manga_failures < MAX_FAILURES {
		trace!("Calling update_page for manga (page {})", random_stats.manga_last_page);
		let page_start_time = chrono::Utc::now();
		has_more_pages = update_page(&mut random_stats, anilist_cache.clone(), false, true).await;
		let page_elapsed = chrono::Utc::now() - page_start_time;
		trace!("update_page for manga completed in {} ms", page_elapsed.num_milliseconds());

		if has_more_pages {
			pages_updated += 1;
			debug!("Successfully updated manga page {}, moving to next page", random_stats.manga_last_page - 1);

			if manga_failures > 0 {
				debug!("Reset manga failure counter from {} to 0 after successful update", manga_failures);
			...
			// If we didn't get more pages, it could be an error or end of data
			manga_failures += 1;
			warn!("Manga update attempt failed for page {}, failure {}/{}", 
	...
	if manga_failures >= MAX_FAILURES {
		error!("Exceeded maximum failures ({}) for manga statistics update", MAX_FAILURES);
		warn!("Some manga statistics may not be up to date");
	}

	let manga_elapsed = chrono::Utc::now() - manga_start_time;
	info!("Completed manga statistics update, processed {} pages in {} seconds", 
		pages_updated, manga_elapsed.num_seconds());
	debug!("Average time per manga page: {} ms", 
		if pages_updated > 0 { manga_elapsed.num_milliseconds() / pages_updated as i64 } else { 0 });

	let total_elapsed = chrono::Utc::now() - start_time;
	info!("Total statistics update completed in {} seconds", total_elapsed.num_seconds());
	debug!("Final stats: anime_last_page={}, manga_last_page={}", 
		random_stats.anime_last_page, random_stats.manga_last_page);
...
async fn update_page(
	...
	debug!("Preparing GraphQL request for {} statistics page {}", page_type, page_num);
	...
	let data = if update_anime {
		...
		trace!("Building AnimeStat GraphQL operation with variables: {:?}", var.page);
		...
		let data: Result<GraphQlResponse<AnimeStat>> =
			make_request_anilist(operation, false, anilist_cache.clone())
				.await
				.with_context(|| format!("Failed to fetch {} statistics for page {}", page_type, page_num));
		...
		trace!("Building MangaStat GraphQL operation with variables: {:?}", var.page);
	...
	debug!("Anilist API request for {} page {} completed in {} ms", 
		page_type, page_num, request_elapsed.num_milliseconds());

	let data = match data {
		Ok(data) => {
			trace!("Successfully received API response for {} page {}", page_type, page_num);
			data
		},
		Err(err) => {
			// Log the error with details about which page failed
			error!("Error updating {} stats for page {}: {:#}", page_type, page_num, err);
			debug!("Error type: {}", std::any::type_name_of_val(&err));

			// Check for specific error patterns
			let err_string = err.to_string();
			if err_string.contains("timeout") {
				warn!("API timeout detected for {} page {}. The external service might be experiencing high load.", 
					page_type, page_num);
			} else if err_string.contains("rate") {
				warn!("Possible rate limiting detected for {} page {}. Consider increasing the update interval.", 
					page_type, page_num);
			} else if err_string.contains("network") || err_string.contains("connection") {
				warn!("Network connectivity issue detected for {} page {}. Check internet connection.", 
					page_type, page_num);
			}
	...
	let has_next_page = match &data.data {
			...
			match &data.site_statistics {
				Some(site_statistics) => {
					...
					match &site_statistics.manga {
						Some(manga) => {
							...
							match &manga.page_info {
								Some(page_info) => {
									...
									if result {
										debug!("Found more pages to process for {} after page {}", page_type, page_num);
									} else {
										debug!("No more pages available for {} after page {}", page_type, page_num);
									}
									result
								},
								None => {
									error!("Missing page_info in API response for {} page {}", page_type, page_num);
						...
						None => {
							error!("Missing manga data in API response for {} page {}", page_type, page_num);
							warn!("API response structure may have changed, check schema");
							trace!("Response data structure: {:?}", data);
							false
						},
				...
				None => {
					error!("Missing site_statistics in API response for {} page {}", page_type, page_num);
					warn!("API response structure may have changed, check schema");
					trace!("Response data structure: {:?}", data);
					false
				},
		...
		None => {
			error!("Empty data in API response for {} page {}", page_type, page_num);
	...
	if has_next_page {
		if update_anime {
			...
			debug!("Incrementing anime_last_page to {}", random_stats.anime_last_page);
		} else {
			random_stats.manga_last_page += 1;
			debug!("Incrementing manga_last_page to {}", random_stats.manga_last_page);
		}
		trace!("Returning true to indicate more pages available");
	} else {
		info!("Reached last page for {} stats: page {}", page_type, page_num);
