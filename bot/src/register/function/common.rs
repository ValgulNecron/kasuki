...
fn add_localised<'a>(
	mut option: CreateCommandOption<'a>, locales: &'a Vec<Localised>,
...
fn add_choices<'a>(
	mut option: CreateCommandOption<'a>, choices: &'a Vec<Choice>,
) -> CreateCommandOption<'a> {
...
fn add_choices_localised<'a>(
	option: CreateCommandOption<'a>, locales: &'a [ChoiceLocalised], name: &'a String,
) -> CreateCommandOption<'a> {
	let vec = locales
		.iter()
		.map(|locale| {
...
pub fn get_permission<'a>(
	permissions: &'a Option<Vec<DefaultPermission>>, mut command_build: CreateCommand<'a>,
) -> CreateCommand<'a> {
