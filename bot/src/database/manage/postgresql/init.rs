...
fn create_and_complete_dot_pgpass(db_config: BotConfigDetails) -> Result<(), Box<dyn Error>> {
    let mut home_dir = dirs::home_dir().ok_or(Box::new(UnknownResponseError::Option(
        String::from("Failed to get home directory."),
    )))?;
    #[cfg(windows)]
    {
    ...
    #[cfg(unix)]
    {
        ...
        let mut permissions = fs::metadata(&pgpass_path)?.permissions();
...
///
/// * A Result that is either an empty Ok variant if the operation was successful, or an Err variant with an AppError if the operation failed.
async fn init_postgres_data(
    pool: &Pool<Postgres>,
    db_config: BotConfigDetails,
) -> Result<(), Box<dyn Error>> {
    // If the database does not exist, create it
    match sqlx::query("CREATE DATABASE kasuki").execute(pool).await {
        Ok(_) => {}
        Err(e) => {
