...
pub async fn set_data_ping_history_postgresql(
    ...
    sqlx::query(
        "INSERT INTO ping_history (shard_id, timestamp, ping) VALUES ($1, $2, $3) \
        ON CONFLICT (shard_id, timestamp) DO UPDATE SET timestamp = EXCLUDED.timestamp, ping = EXCLUDED.ping, shard_id = EXCLUDED.shard_id",
    )
...
pub async fn get_user_color_count_postgresql(
    db_config: BotConfigDetails,
