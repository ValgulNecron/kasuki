...
impl SlashCommand for SkipCommand {
	async fn run_slash(&self) -> anyhow::Result<()> {
		...
		let Some(player) =
			...
			let content = EmbedContent::new(skip_localised.title)
				.description(skip_localised.error_no_voice)
				.command_type(EmbedType::Followup);
			return self.send_embed(vec![content]).await;
		};
		let mut content = EmbedContent::new(skip_localised.title).command_type(EmbedType::Followup);

		let now_playing = player.get_player().await?.track;

		if let Some(np) = now_playing {
			player.skip()?;
			content.description = skip_localised.success.replace("{0}", &np.info.title);
		} else {
			content.description = skip_localised.nothing_to_skip;
		}
