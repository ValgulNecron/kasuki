...
impl_command!(
	...
	get_contents = |self_: FishInventoryCommand| async move {
		...
		if inventory_items.is_empty() {
			...
			let embed_content = EmbedContent::new(USABLE_LOCALES.lookup(&lang_id, "minigame_fish_inventory-title"))
				.description(USABLE_LOCALES.lookup(&lang_id, "minigame_fish_inventory-empty_description"));
		...
		let mut embed_content = EmbedContent::new(USABLE_LOCALES.lookup(&lang_id, "minigame_fish_inventory-title"))
		...
		for (fish_name, fish_list) in &fish_by_name {
			...
			fish_summary.push_str(&USABLE_LOCALES.lookup_with_args(&lang_id, "minigame_fish_inventory-fish_format", &args));
		}
		...
		fields.push((USABLE_LOCALES.lookup(&lang_id, "minigame_fish_inventory-fish_by_type"), fish_summary, false));
		...
		for (fish_name, fish_list) in &fish_by_name {
			...
			if let Some((inventory_item, _)) = sorted_fish.first() {
				...
				fish_details.push_str(&USABLE_LOCALES.lookup_with_args(&lang_id, "minigame_fish_inventory-specimen_format", &args));
			}
		}

		// Add fish details to the embed
		fields.push((USABLE_LOCALES.lookup(&lang_id, "minigame_fish_inventory-best_specimens"), fish_details, false));
		...
		for (inventory_item, _) in &inventory_items {
			*rarity_distribution_map.entry(inventory_item.rarity).or_insert(0) += 1;
		}
		...
		for rarity in 1..=5 {
			...
			rarity_text.push_str(&USABLE_LOCALES.lookup_with_args(&lang_id, "minigame_fish_inventory-rarity_format", &args));
		}

		// Add rarity distribution to the embed
		fields.push((USABLE_LOCALES.lookup(&lang_id, "minigame_fish_inventory-rarity_distribution"), rarity_text, false));
		...
		count_args.insert(Cow::Borrowed("count"), FluentValue::from(total_fish_count.to_string()));

		let mut value_args: HashMap<Cow<'static, str>, FluentValue> = HashMap::new();
		value_args.insert(Cow::Borrowed("value"), FluentValue::from(total_fish_value.to_string()));

		fields.push((
			USABLE_LOCALES.lookup(&lang_id, "minigame_fish_inventory-summary"),
			format!(
				"{}\n{}",
				USABLE_LOCALES.lookup_with_args(&lang_id, "minigame_fish_inventory-total_fish", &count_args),
				USABLE_LOCALES.lookup_with_args(&lang_id, "minigame_fish_inventory-total_value", &value_args)
			),
			false
		));
