...
async fn main() -> Result<()> {
    ...
    tokio::spawn(async move {
        ...
        let mut interval = tokio::time::interval(Duration::from_secs(intervals_clone_3.activity_check));
        loop {
            interval.tick().await;
            manage_activity(http_clone.clone(), cache_clone_2.clone(), db_clone_2.clone()).await;
        }
    });
...
async fn update_anisong_db(db: Arc<DatabaseConnection>, task_intervals: TaskIntervalConfig) {
    ...
    loop {
        ...
        match get_anisong(db.clone()).await {
            Ok(count) => info!("Anisong database update cycle #{} completed. Processed: {}", update_count, count),
            Err(e) => error!("Anisong database update cycle #{} failed: {:#}", update_count, e),
        }
    }
}
