...
pub async fn load_localization_info(guild_id: String) -> Result<InfoLocalised, CommandError> {
    let mut file = File::open("json/message/general/info.json")
        .map_err(|e| NotFound(format!("File info.json not found. {}", e)))?;

    let mut json = String::new();
    file.read_to_string(&mut json)
        .map_err(|e| Reading(format!("File info.json can't be read. {}", e)))?;

    let json_data: HashMap<String, InfoLocalised> = serde_json::from_str(&json)
        .map_err(|e| Parsing(format!("Failing to parse info.json. {}", e)))?;
