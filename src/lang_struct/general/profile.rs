...
pub async fn load_localization_profile(guild_id: String) -> Result<ProfileLocalised, CommandError> {
    let mut file = File::open("json/message/general/profile.json")
        .map_err(|e| NotFound(format!("File profile.json not found. {}", e)))?;

    let mut json = String::new();
    file.read_to_string(&mut json)
        .map_err(|e| Reading(format!("File profile.json can't be read. {}", e)))?;

    let json_data: HashMap<String, ProfileLocalised> = serde_json::from_str(&json)
        .map_err(|e| Parsing(format!("Failing to parse profile.json. {}", e)))?;
