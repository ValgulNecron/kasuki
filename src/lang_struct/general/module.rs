...
pub async fn load_localization_module_activation(
    guild_id: String,
) -> Result<ModuleLocalised, CommandError> {
    let mut file = File::open("json/message/general/module.json")
        .map_err(|e| NotFound(format!("File module.json not found. {}", e)))?;

    let mut json = String::new();
    file.read_to_string(&mut json)
        .map_err(|e| Reading(format!("File module.json can't be read. {}", e)))?;

    let json_data: HashMap<String, ModuleLocalised> = serde_json::from_str(&json)
        .map_err(|e| Parsing(format!("Failing to parse module.json. {}", e)))?;
