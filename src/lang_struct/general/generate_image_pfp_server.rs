...
pub async fn load_localization_pfp_server_image(
    ...
    let mut file =
        File::open("json/message/general/generate_image_pfp_server.json").map_err(|_| {
            LocalisationFileError(String::from(
    ...
    file.read_to_string(&mut json).map_err(|_| {
        LocalisationReadError(String::from(
    ...
    let json_data: HashMap<String, PFPServerLocalisedImage> =
        serde_json::from_str(&json).map_err(|_| {
            LocalisationParsingError(String::from(
