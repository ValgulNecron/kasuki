...
pub async fn load_localization_random_image_nsfw(
    guild_id: String,
) -> Result<RandomImageNSFWLocalised, CommandError> {
    let mut file = File::open("json/message/anilist/random_image_nsfw.json")
        .map_err(|e| NotFound(format!("File random_image_nsfw.json not found. {}", e)))?;

    let mut json = String::new();
    file.read_to_string(&mut json)
        .map_err(|e| Reading(format!("File random_image_nsfw.json can't be read. {}", e)))?;

    let json_data: HashMap<String, RandomImageNSFWLocalised> = serde_json::from_str(&json)
        .map_err(|e| Parsing(format!("Failing to parse random_image_nsfw.json. {}", e)))?;
