...
pub async fn load_localization_staff(guild_id: String) -> Result<StaffLocalised, CommandError> {
    let mut file = File::open("json/message/anilist/staff.json")
        .map_err(|e| NotFound(format!("File staff.json not found. {}", e)))?;

    let mut json = String::new();
    file.read_to_string(&mut json)
        .map_err(|e| Reading(format!("File staff.json can't be read. {}", e)))?;

    let json_data: HashMap<String, StaffLocalised> = serde_json::from_str(&json)
        .map_err(|e| Parsing(format!("Failing to parse staff.json. {}", e)))?;
