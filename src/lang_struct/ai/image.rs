...
use crate::error_enum::Error::{LocalisationFileError, LocalisationParsingError, LocalisationReadError, NoLangageError};
...
pub async fn load_localization_image(guild_id: String) -> Result<ImageLocalised, AppError> {
    let mut file = File::open("json/message/ai/image.json")
        .map_err(|e| Error(LocalisationFileError(format!("File image.json not found. {}", e))))?;

    let mut json = String::new();
    file.read_to_string(&mut json)
        .map_err(|e| Error(LocalisationReadError(format!("File image.json can't be read. {}", e))))?;

    let json_data: HashMap<String, ImageLocalised> = serde_json::from_str(&json)
        .map_err(|e| Error(LocalisationParsingError(format!("Failing to parse image.json. {}", e))))?;
