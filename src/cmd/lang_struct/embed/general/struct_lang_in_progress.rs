...
impl InProgressLocalisedText {
    pub async fn get_in_progress_localised(
        color: Colour,
        ctx: &Context,
        command: &ApplicationCommandInteraction,
    ) -> Result<InProgressLocalisedText, &'static str> {
        let mut file = match File::open("lang_file/embed/general/in_progress.json") {
            Ok(file) => file,
            Err(_) => {
        ...
        let json_data: HashMap<String, InProgressLocalisedText> = match serde_json::from_str(&json)
        {
            Ok(data) => data,
            Err(_) => {
