...
type RegisterLocalisedLangList = HashMap<String, LangRegister>;
...
impl LangRegister {
    ...
    pub fn get_profile_register_localised() -> Result<RegisterLocalisedLangList, &'static str> {
        let mut file = match File::open("lang_file/command_register/general/lang.json") {
            Ok(file) => file,
