...
pub async fn migrate_sqlite() -> Result<(), DatabaseError> {
...
pub async fn add_new_member_to_module_activation() -> Result<(), DatabaseError> {
    let pool = get_sqlite_pool(DATA_SQLITE_DB).await?;
    ...
    if row == 0 {
        sqlx::query("ALTER TABLE module_activation ADD COLUMN new_member TEXT")
            .execute(&pool)
            .await
            .map_err(|e| Alter(format!("Failed to add column to the table. {}", e)))?;
    }
