...
impl SiteStatisticsAnimeWrapper {
    pub async fn new_anime(
        page_number: i64,
    ) -> Result<(SiteStatisticsAnimeWrapper, String), WebRequestError> {
        ...
        let api_response: SiteStatisticsAnimeWrapper = serde_json::from_str(&res)
            .map_err(|e| NotFound(format!("No media with page {}. {}", page_number, e)))?;
        Ok((api_response, res))
    }
